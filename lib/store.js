...
function loadDb() {
  ...
  if (!db.videoAssignments || typeof db.videoAssignments !== 'object') db.videoAssignments = {};
  if (!db.videoProgress || typeof db.videoProgress !== 'object') db.videoProgress = {};
  for (const playlist of db.playlists) {
    ...
    playlist.targetDays = Math.max(1, Number.parseInt(playlist.targetDays, 10) || Math.ceil(playlist.totalVideos / (db.settings?.videoDailyCount || 2)));
  ...
  for (const assignment of Object.values(db.videoAssignments)) {
    const seen = new Set();
