...
function getOrCreateVideoAssignment(date) {
  ...
  while (items.length < count) {
    const candidates = playlists.map((playlist) => {
      ...
      // A video is not consumed merely because it was once placed in a plan.
      // Always return to the earliest unfinished lesson; only completion moves
      // the playlist forward, preventing pending lessons from being skipped.
...
async function addPlaylist({ title, url, totalVideos, targetDays }) {
  ...
  let metadata;
  const key = db.settings.youtubeApiKey;
