...
function getOrCreateVideoAssignment(date) {
  ...
  while (items.length < count) {
    ...
    candidates.sort((a, b) => b.urgency - a.urgency ||
      (usedPlaylists.has(a.playlist.id) ? 1 : 0) - (usedPlaylists.has(b.playlist.id) ? 1 : 0) || a.position - b.position);
...
async function addPlaylist({ title, url, totalVideos, targetDays }) {
  ...
  delete db.videoAssignments[require('./store').todayStr()];
  saveDb();
  return { playlist };
}
...
module.exports = { getOrCreateVideoAssignment, setVideoStatus, addPlaylist, syncPlaylist, removePlaylist, playlistVideoUrl, progressFor, VIDEO_STATUSES };
