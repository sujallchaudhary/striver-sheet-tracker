...
const { getDb, saveDb, STATUS_LABEL, todayStr, currentUserId, runForUser } = require('./store');
...
// One timer per user: the callback fires after the request that scheduled it
// has ended, so it re-enters that user's scope with freshly loaded state.
...
function scheduleAutoSync() {
  ...
  syncTimers.set(userId, setTimeout(() => {
    ...
    runForUser(userId, () => syncDay(todayStr()))
      .catch((err) => console.error('Notion auto-sync failed:', err.message));
  }, 3000));
}
