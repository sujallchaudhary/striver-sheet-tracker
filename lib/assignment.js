...
const REVISION_COOLDOWN_DAYS = 2;   // revision_needed comes back after this many days
...
function getOrCreateAssignment(date) {
  ...
  const dueForRevision = db.problems
    .filter((p) => {
      ...
      if (p.status === 'revision_needed') return age >= REVISION_COOLDOWN_DAYS;
    ...
    .sort((a, b) => {
      ...
      return ra - rb || a.statusUpdatedAt.localeCompare(b.statusUpdatedAt) || a.order - b.order;
    });
  ...
  if (items.length < revisionTarget && s.revisionTopics.length) {
    const fallback = db.problems
      .filter((p) =>
        ...
        ['completed', 'solved_with_help', 'revision_needed'].includes(p.status) &&
